...
<xsl:stylesheet
	...
	<xsl:template name="summary">
		...
		<table align="center" class="details" border="0" cellpadding="5"
			...
			<tr valign="top">
				<xsl:variable name="allCount"
					select="count(/testResults/*)" />
				...
				<xsl:variable name="allSuccessPercent"
					select="$allSuccessCount div $allCount" />
				<xsl:variable name="allTotalTime"
					select="sum(/testResults/*/@t)" />
				<xsl:variable name="allAverageTime"
					select="$allTotalTime div $allCount" />
				<xsl:variable name="allMinTime">
					<xsl:call-template name="min">
						<xsl:with-param name="nodes"
							select="/testResults/*/@t" />
					</xsl:call-template>
				</xsl:variable>
	...
	<xsl:template name="pagelist">
		...
		<table align="center" class="details" border="0" cellpadding="5"
			...
			<xsl:for-each
				select="/testResults/*[not(@lb = preceding::*/@lb)]">
				<xsl:variable name="label" select="@lb" />
				<xsl:variable name="count"
					select="count(../*[@lb = current()/@lb])" />
				...
				<xsl:variable name="successPercent"
					select="$successCount div $count" />
				<xsl:variable name="totalTime"
					select="sum(../*[@lb = current()/@lb]/@t)" />
				<xsl:variable name="averageTime"
					select="$totalTime div $count" />
				<xsl:variable name="minTime">
					<xsl:call-template name="min">
						<xsl:with-param name="nodes"
							select="../*[@lb = current()/@lb]/@t" />
					</xsl:call-template>
				</xsl:variable>
		...
		<xsl:if test="$allFailureCount > 0">
			...
			<xsl:for-each
				...
				<xsl:if test="$failureCount > 0">
					...
					<table align="center" class="details" border="0"
						...
						<xsl:for-each
							...
							<tr>
								...
								<td>
									<xsl:value-of
										select="assertionResult/failureMessage" />
								</td>
								<xsl:if test="$showData = 'y'">
									<td>
